...
(function () {
  ...
  function chatsScreen() {
    ...
    const rows = accepted
      .map((c) => ({ c, thread: s.threads.find((t) => t.contactId === c.id) }))
      .sort((a, b) => (b.thread ? b.thread.updatedAt : 0) - (a.thread ? a.thread.updatedAt : 0))
      .map(({ c, thread }) => {
  ...
  // -------------------- Thread --------------------
  function threadScreen(contactId) {
    const s = S.state;
    ...
    const msgs = thread.id ? S.messagesForThread(thread.id) : [];
    ...
    const emojiStrip = !isPending ? `
      <div class="emoji-strip">
        ${['👍','❤️','😂','😮','🙏','🔥'].map((e) => `<button data-action="react" data-emoji="${e}">${e}</button>`).join('')}
      </div>` : '';

    return `
      <div class="thread">${bubbles || '<div class="empty-state"><p>No messages yet.</p></div>'}</div>
      ${replyLock}
      ${emojiStrip}
      <div class="composer">
        <input type="text" id="composer-input" placeholder="Message ${esc(contact.name)}" ${isPending ? 'disabled' : ''}>
        <button class="icon-btn" data-action="send" ${isPending ? 'disabled' : ''}>${icon('send')}</button>
      </div>
    `;
  }
  ...
  function threadHeaderActions(contactId) {
    const contact = S.contactById(contactId);
    if (!contact) return '';
    return `<button class="icon-btn" data-action="open-thread-menu" data-id="${contact.id}">${icon('more')}</button>`;
  }
